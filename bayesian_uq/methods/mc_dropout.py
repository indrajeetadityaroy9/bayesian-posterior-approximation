...
class MCDropoutConfig(UQMethodConfig):
...
class MCDropout(BaseUQMethod):
    config_class = MCDropoutConfig
    ...
    def __init__(self, config=None):
        ...
        mlp_config.uncertainty_method = "mc_dropout"
        mlp_config.mc_samples = self.config.mc_samples
    ...
    def predict_with_uncertainty(self, X):
        ...
        with torch.no_grad():
            (mean_probs, variance, log_probs) = self.model.mc_forward(
                X_tensor, num_samples=self.config.mc_samples
            )
            predictions = to_numpy(mean_probs.argmax(dim=1))
            probabilities = to_numpy(mean_probs)
            total_uncertainty = to_numpy(variance.sum(dim=1))
            entropy = -(mean_probs * torch.log(mean_probs.clamp_min(1e-08))).sum(dim=1)
            entropy = to_numpy(entropy)
            logits = to_numpy(log_probs)
    ...
    def load(self, path):
        ...
        if self.model is None:
            from bayesian_uq.core.models import MCDropoutMLP

            self.model = MCDropoutMLP(self.mlp_config)
